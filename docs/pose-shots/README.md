# Historical pose defects, fixed

These captures show the five defect classes that were present before the
wrist and reach correction. They are retained as regression evidence. The
current deterministic sweep reports zero violating samples and the acceptance
list is empty; regenerate only when the audit finds a new failure.

Generated by `npm run audit:visual`. Each entry is the worst sample of one
defect that `npm run audit:npc` reports, with the scene frozen at that exact
moment of that exact action. Two angles per defect: a three-quarter view and
a side view, because a limb inside the counter is invisible from the front.

Regenerate after changing a pose. The measurements come from the audit, so
they are the same numbers CI asserts on.

One shot per owning issue is committed here. `npm run audit:visual -- --all`
photographs every distinct defect instead, which is the one to use when
working through a single issue.

## geometry interpenetration — #29

**handL is 0.113m inside the pot**

cook0 performing `stir` at t=2.4s · id `clip:cook0:stir:handL:pot`

![clip:cook0:stir:handL:pot](clip-cook0-stir-handL-pot.png)

![clip:cook0:stir:handL:pot from the side](clip-cook0-stir-handL-pot-side.png)

## geometry interpenetration — #28

**forearmL is 0.085m inside the counter top**

diner2 performing `drink` at t=0s · id `clip:diner2:drink:forearmL:counter-top`

![clip:diner2:drink:forearmL:counter-top](clip-diner2-drink-forearmL-counter-top.png)

![clip:diner2:drink:forearmL:counter-top from the side](clip-diner2-drink-forearmL-counter-top-side.png)

## contact miss — #30

**cloth hovers 0.046m above the counter it is wiping**

cook0 performing `wipe` at t=0s · id `contact:cook0:wipe:cloth-counter`

![contact:cook0:wipe:cloth-counter](contact-cook0-wipe-cloth-counter.png)

![contact:cook0:wipe:cloth-counter from the side](contact-cook0-wipe-cloth-counter-side.png)

## contact miss — #31

**chopsticks stop 0.035m short of the face at the top of the bite**

diner2 performing `eat` at t=2.15s · id `contact:diner2:eat:chopsticks-mouth`

![contact:diner2:eat:chopsticks-mouth](contact-diner2-eat-chopsticks-mouth.png)

![contact:diner2:eat:chopsticks-mouth from the side](contact-diner2-eat-chopsticks-mouth-side.png)

## geometry interpenetration — #32

**forearmL is 0.011m inside the counter front**

diner1 performing `drink` at t=0s · id `clip:diner1:drink:forearmL:counter-front`

![clip:diner1:drink:forearmL:counter-front](clip-diner1-drink-forearmL-counter-front.png)

![clip:diner1:drink:forearmL:counter-front from the side](clip-diner1-drink-forearmL-counter-front-side.png)
